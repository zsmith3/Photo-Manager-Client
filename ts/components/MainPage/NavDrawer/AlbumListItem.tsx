...
export default class AlbumListItem extends MountTrackedComponent<{ albumId: number, indent?: number }> {
	...
	constructor (props: { albumId: number, indent?: number }) {
		...
		this.updates = { subName: "", parent: this.state.album.parent !== null ? this.state.album.parent.id : null };
	}
		...
		return <Fragment>
				...
				<Fragment>
					...
					<TextDialog
						...
						label="Album Name" defaultValue={ this.state.album.name }
					...
					<ListDialog
						...
						selected={ this.state.album.parent === null ? null : this.state.album.parent.id } nullItem="/"
						action={ (parentId: number) => Album.getById(this.props.albumId).updateSave({ parent: parentId }).then(() => Album.handleListUpdate()) }
					/>

					{/* Delete album dialog */}
					<SimpleDialog
						open={ this.state.openDialogRemove } onClose={ () => this.dialogClose("Remove") }
						title="Remove Album" text={ <Fragment>Are you sure you want to delete the album <i>{ this.state.album.name }</i>?</Fragment> } actionText="Confirm"
