...
export default class PersonGroupListItem extends MountTrackedComponent<{ groupId: number }> {
	state = {
			...
			<Fragment>
				...
				<Menu anchorEl={ this.state.menuAnchorEl } open={ this.state.openMenu } onClick={ this.menuClose } onClose={ this.menuClose }>
					<MenuList subheader={ <ListSubheader style={ { lineHeight: "24px" } }>{ this.state.group.name }</ListSubheader> }>
						...
						<MenuItem onClick={ () => this.dialogOpen("New") }><ListItemIcon><Icon>add</Icon></ListItemIcon>New Person</MenuItem>
				...
				<TextDialog
					...
					title="Rename Group" actionText="Rename"
					label="Group Name" defaultValue={ this.state.group.name }
				...
				<TextDialog
					...
					title="Create Person" actionText="Create" label="Person Name"
				...
				<SimpleDialog
					...
					title="Remove Group" text={ <Fragment>Are you sure you want to delete the person <i>{ this.state.group.name }</i>?</Fragment> } actionText="Confirm"
