...
class ImageModal extends HTMLElement {
	connectedCallback () {
		...
		setTimeout(function () {
			...
			_this.hammerApi.on("swipe", function (event) {
				if (App.app.config.platform == "mobile") {
	...
	switchFile (direction) {
		if (App.app.data.objectType == "files") {
			let nextFile = App.app.els.filesCont.getAdjacentFile(this.file, direction, "image");
			if (nextFile) this.openFile(nextFile);
		} else if (App.app.data.objectType == "faces") {
			let nextFile = App.app.els.filesCont.getAdjacentFile({id: this.file.parent}, direction).file;
	...
	displayArrows () {
		...
		if (App.app.config.platform == "mobile") return;

		if (App.app.data.objectType == "files") {
			if (App.app.els.filesCont.getAdjacentFile(this.file, -1, "image")) {
			...
			if (App.app.els.filesCont.getAdjacentFile({id: this.file.parent}, -1)) {
				$(this).find("#im-arrow-left").css("display", "");
			}
			if (App.app.els.filesCont.getAdjacentFile({id: this.file.parent}, 1)) {
				$(this).find("#im-arrow-right").css("display", "");
	...
	setZoom (maxW, maxH, xPos, yPos) {
		...
		else if (maxW == "min") maxW = window.innerWidth - (App.app.config.platform == "mobile" ? 0 : 300);
		if (maxH == "max") maxH = this.file.height;
		else if (maxH == "min") maxH = window.innerHeight - (App.app.config.platform == "mobile" ? 0 : $(this).find("#im-toolbar").get(0).clientHeight);
		// TODO faces support

		//maxW = (maxW || this.file.zoom.maxW) || (window.innerWidth - (App.app.config.platform == "mobile" ? 0 : 300));
		//maxH = (maxH || this.file.zoom.maxH) || (window.innerHeight - (App.app.config.platform == "mobile" ? 0 : $(this).find("#im-toolbar").get(0).clientHeight)) * (App.app.config.platform == "mobile" ? 1 : 0.9);
		...
		if (yPos == "c") yPos = (window.innerHeight - newHeight + (App.app.config.platform == "mobile" ? 0 : $(this).find("#im-toolbar").get(0).clientHeight)) / 2;

		// TODO faces support
		//xPos = (xPos || this.file.zoom.xPos) || (window.innerWidth - newWidth) / 2;
		//yPos = (yPos || this.file.zoom.yPos) || (window.innerHeight - newHeight + (App.app.config.platform == "mobile" ? 0 : iPTopBar.clientHeight)) / 2;
	...
	selectCurrentFile () {
		App.app.els.filesCont.selectAll(false);
		let fbox = App.app.els.filesCont.getFilebox(this.file.id);
		fbox.selected = true;
