...
class GMapEdit extends GoogleMap {
	...
	postInit () {
		...
		} else if (pageLoader.filesContainer.selection.length == 1) {
			$("#modal-geotag-form-location-single").css("display", "");

			let geotag = pageLoader.filesContainer.getFile(pageLoader.filesContainer.selection[0]).geotag;
			if (geotag != null) {
		...
		} else if (pageLoader.filesContainer.selection.length == 0) {
