		...
		return this.getAllSyncFiles().then(function(data: { localFiles: object; remoteFiles: object }) {
			...
			return Platform.files
				...
				.progress(function(data) {
					sync.notify({
						text: "Deleting " + (data.doneCount + 1) + " of " + data.totalCount + " files",
				...
				.then(function() {
					...
					return Platform.files
						...
						.progress(function(data) {
							sync.notify({
								text: "Moving " + (data.doneCount + 1) + " of " + data.totalCount + " files",
								progress: (data.doneCount / data.totalCount) * 100
							});
						});
				})
	...
	getAllSyncFiles() {
		...
		return new Promise<{ localFiles: object; remoteFiles: object }>(function(resolve, reject) {
			...
			Platform.files.getLocalFiles(sync.localDir).then(function(files) {
				allLocalFiles = files.reduce(
					(allFiles, filePath) =>
						Object.assign(allFiles, {
							[filePath
								.split("/")
								.reverse()[0]
								.split(".")[0]]: decodeURI(filePath.substr(sync.localDir.length))
						}),
					{}
				);
