...
import SyncPair from "../controllers/SyncPair"

/** Separate page for logging in */
export default class SyncPage extends React.Component<{ history: any }> {
	state = {
		syncPairs: SyncPair.getAll()
	}

	onChangeRemote(index: number, value: string) {
		this.setState({syncPairs: [...this.state.syncPairs.slice(0, index), new SyncPair(this.state.syncPairs[index].localDir, value), ...this.state.syncPairs.slice(index + 1)]})
	} 

	save() {
		SyncPair.save(this.state.syncPairs);
	}

	render() {
		return (
			<GenericPage title="Local Sync" introText="Set up synchronisation between remote fileserver and local storage" showBackArrow>
				<br />
				
				{this.state.syncPairs.map((syncPair, index) => <div>
					<Typography>Local folder:</Typography>
					{/* TODO figure out how to use the file picker (or find another one) */}
					<TextField label="Remote Folder" onChange={event => this.onChangeRemote(index, event.target.value)} />
...
// TODO remove code below (once file picker done)
